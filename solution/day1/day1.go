...
func TotalFuel(mass []float64) int {
	var total = 0
...
What is the sum of the fuel requirements for all of the modules on your spacecraft
when also taking into account the mass of the added fuel? (Calculate the fuel requirements for each module separately,
then add them all up at the end.)
*/
func TotalFuelRequirements(mass []float64) int {
	var total = 0

	for _, v := range mass {
		total += fuelRequirements(v)
	}

	return total
}

func fuelRequirements(fuel float64) int {
	var total = 0
	var current = int(fuel)

	for current != 0 {
		part := step(float64(current))
